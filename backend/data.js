/* 数据生成模块：确定性种子随机，保证重启后数据稳定（便于前端联调与截图） */
...
export const SCENES = ['厨房', '工厂', '商超', '客厅', '仓库', '办公室'];
export const TASKS = ['抓取', '放置', '折叠衣物', '开门', '倒水', '插拔', '擦拭', '分拣'];
export const OBJECTS = ['红色杯子', '衣物', '螺丝刀', '抽屉', '水瓶', '盘子', '纸箱', '开关'];
export const ROBOTS = ['单臂夹爪', '双臂', '人形', '移动底盘+臂'];
export const SOURCES = ['真机遥操', '无本体采集', '仿真', '第三方导入'];
export const DEVICES = ['MEgo-View', 'MEgo-Gripper', 'TeleXperience', 'iPhone-Pro-Rig', 'SimEngine'];
export const USERS = ['张伟', '李娜', '王强', '刘洋', '陈静', '赵磊'];
export const MODALITIES = ['rgb', 'depth', 'imu', 'force', 'pose', 'audio', 'pointcloud'];
export const GRADES = ['A', 'B', 'C'];
export const ENVS = ['白天', '夜晚', '强光', '遮挡'];

export const STATUS_FLOW = [
  { key: 'uploaded', label: '已上传' }, { key: 'scanned', label: '已入库' },
  { key: 'raw', label: '原始待处理' }, { key: 'cleaning', label: '清洗中' },
  { key: 'preprocessed', label: '已预处理' }, { key: 'annotating', label: '标注中' },
  { key: 'annotated', label: '已标注' }, { key: 'asset', label: '数据资产' },
  { key: 'in_dataset', label: '已进数据集' }, { key: 'training', label: '训练中' },
  { key: 'archived', label: '归档' },
];
export const STATUS_LABEL = Object.fromEntries(STATUS_FLOW.map((s) => [s.key, s.label]));
const STATUS_IDX = Object.fromEntries(STATUS_FLOW.map((s, i) => [s.key, i]));

function randomModalities() {
  const base = ['rgb', 'pose'];
  MODALITIES.forEach((m) => { if (!base.includes(m) && rng() > 0.45) base.push(m); });
  return base;
}
function randomDate() {
...
for (let i = 1; i <= 320; i++) {
  ...
  if (grade === 'C' && rng() > 0.6) status = 'cleaning';
  ...
  EPISODES.push({
    ...
    source, scene, task, object: obj, robot, status,
    duration: +(rng() * 110 + 8).toFixed(1),
    fps: rand([20, 30, 60]),
    frames: randInt(300, 3600),
    cameras: randInt(1, 4),
    modalities: randomModalities(),
    action_dim: rand([6, 7, 12, 14]),
    quality_score: score, quality_grade: grade, success,
    collected_by: rand(USERS), device: rand(DEVICES),
    collected_at: date.toISOString().slice(0, 10),
    size_gb: +(rng() * 9 + 0.5).toFixed(2),
    tags: [scene, task, obj, robot, source, success ? '成功' : '失败', rand(ENVS)],
    version: randInt(1, 4),
  });
}
...
export const CLEAN_RULES = [
  { rule: '丢帧超阈', count: 42 }, { rule: '时间戳错乱', count: 18 }, { rule: '时长过短', count: 27 },
  { rule: '传感器缺失', count: 12 }, { rule: '近重复', count: 9 }, { rule: '力/位姿突变', count: 15 },
];
...
export function buildStats() {
  return {
    ...
    trend: (() => {
      const m = {};
      EPISODES.forEach((e) => { const mo = e.collected_at.slice(0, 7); m[mo] = (m[mo] || 0) + 1; });
    ...
    funnel: (() => {
      ...
      return order.map((k) => ({ stage: STATUS_LABEL[k], value: EPISODES.filter((e) => STATUS_IDX[e.status] >= STATUS_IDX[k]).length }));
    })(),
    flywheel: [
